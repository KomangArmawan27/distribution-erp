...
class InvoiceHeader(Base):
    ...
    doctype_id: Mapped[int] = mapped_column(SmallInteger, nullable=False, default=3)
    ...
    details: Mapped[list["InvoiceDetail"]] = relationship(
        "InvoiceDetail", back_populates="header", cascade="all, delete-orphan", lazy="selectin"
    )
    ...
    def invoice_total(self) -> Decimal:
        return sum((d.trans_total for d in self.details), Decimal("0.00"))


class InvoiceDetail(Base):
    __tablename__ = "invoice_detail"
    __table_args__ = (
        UniqueConstraint("invoice_id", "item_id", name="uq_invoice_detail_doc_item"),
        CheckConstraint("trans_qty > 0", name="ck_invoice_detail_trans_qty_positive"),
        {"schema": "sales"},
    )

    trans_id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    invoice_id: Mapped[int] = mapped_column(
        ForeignKey("sales.invoice_header.invoice_id", ondelete="CASCADE"), nullable=False
    )
    ...
    item_price: Mapped[Decimal] = mapped_column(Numeric(15, 2), nullable=False)
    trans_total: Mapped[Decimal] = mapped_column(Numeric(15, 2), nullable=False)

    header: Mapped["InvoiceHeader"] = relationship("InvoiceHeader", back_populates="details")
